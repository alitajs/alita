 ...
 * @return {EventEmitter}
 */
function setupEvents(externalEventEmitter: any) {
 ...
 */
function cordovaCreate(dest: any, opts = {} as any) {
  ...
  return Promise.resolve().then(function () {
    ...
    .then(function (input_directory) {
      ...
      let isSubDir = false;
      try {
        const templatePkg = requireFresh(input_directory) as any;
 ...
 */
function copyIfNotExists(src: string, dst: any) {
 ...
 */
function projectConfig(projectDir: string) {
 ...
 * */
function linkFromTemplate(templateDir: string, projectDir: string) {
  let linkSrc, linkDst, linkFolders, copySrc, copyDst;
  function rmlinkSync(src: any, dst: any, type: any) {
    if (src && dst) {
      fs.removeSync(dst);
