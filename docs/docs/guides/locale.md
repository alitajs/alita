...
它可以轻松地将国际化功能集成到你的应用程序之中。

> 文档和方案来自 https://umijs.org/docs/max/i18n ，只是为了便于使用整合到这里
...
一切就绪，现在您可以使用多语言内容。交给我们的 `<FormattedMessage />` 组件吧，只需要将前面的 `welcome` 作为参数 `id` 的值传入即可：

```tsx
import { FormattedMessage } from 'alita';
...
```ts
import { getAllLocales } from 'alita';
