...
可以通过环境变量来完成一些特殊的配置和功能。

## 如何设置环境变量

### 执行命令时设置

例如需要改变 `alita dev` 开发服务器的端口，进可以通过如下命令实现。
...
`alita` 会以 3000 端口启动 dev server，并且禁用 babel 的缓存。
...
`alita` 会以 4000 端口启动 dev server，同时保持禁用 babel 的缓存。
...
```bash
$ SOCKET_SERVER=https://localhost:7001/ alita dev
```
...
```bash
$ SPEED_MEASURE=JSON alita dev
```

### UMI_PLUGINS

指定 `alita` 命令执行时额外加载的插件的路径，使用 `,` 隔开。

```bash
$ UMI_PLUGINS=./path/to/plugin1,./path/to/plugin2  alita dev
```

### UMI_PRESETS

指定 `alita` 命令执行时额外加载插件集的路径，使用 `,` 隔开。

```bash
$ UMI_PRESETS=./path/to/preset1,./path/to/preset2  alita dev
```
...
```bash
$ WEBPACK_FS_CACHE_DEBUG=1 alita dev
```
